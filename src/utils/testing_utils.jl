using Random
"""
    make_random_network(layer_sizes::Vector{Int, 1}, [min_weight = -1.0], [max_weight = 1.0], [min_bias = -1.0], [max_bias = 1.0], [rng = 1.0])
...
"""
function make_random_network(layer_sizes::Vector{Int, 1}, min_weight = -1.0, max_weight = 1.0, min_bias = -1.0, max_bias = 1.0, rng=MersenneTwister(0))
    ...
    for index in 1:(length(layer_sizes)-1)
        ...
        cur_weights = min_weight + (max_weight - min_weight) * rand(rng, Float64, (layer_sizes(index+1), layer_sizes(index)))
        cur_weights = reshape(cur_weights, (layer_sizes(index+1), layer_sizes(index))) # for edge case where 1 dimension is equal to 1 this keeps it from being a 1-d vector

        # Dimension: num_out x 1
        cur_bias = min_weight + (max_weight - min_weight) * rand(rng, Float64, (layer_sizes(index+1)))
