...
solver = NSVerify(optimizer=Cbc.Optimizer)
println("NSVerify - small_nnet")
...
#optimizer = GLPKSolverMIP()
solver = MIPVerify(optimizer=Cbc.Optimizer)
println("MIPVerify - small_nnet")
timed_result =@timed solve(solver, problem_holds)
println(" - Time: " * string(timed_result[2]) * " s")
println(" - Output: ")
println(timed_result[1])
...
#optimizer = GLPKSolverMIP()
solver = ILP(optimizer=Cbc.Optimizer)
println("ILP - small_nnet")
timed_result =@timed solve(solver, problem_holds)
println(" - Time: " * string(timed_result[2]) * " s")
println(" - Output: ")
println(timed_result[1])
